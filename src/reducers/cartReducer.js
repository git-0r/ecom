const cartReducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case "ADD_TO_CART":
            ...
            return { products: [...state.products, productToAdd], total: state.total + productToAdd.price }

        case "INCREASE_IN_CART":

            const productToIncrease = action.payload;

            const productsAfterIncrease = state.products.map(
                product => product._id === productToIncrease._id
                    ? { ...product, quantity: product.quantity + 1 }
                    : product
            )
            return { products: productsAfterIncrease, total: state.total + productToIncrease.price };

        case "DECREASE_IN_CART":

            const productToDecrease = action.payload;
            const productsAfterDecrease = state.products.map(
                product => product._id === productToDecrease._id
                    ? { ...product, quantity: product.quantity - 1 }
                    : product
            )
            return { products: productsAfterDecrease, total: state.total - productToDecrease.price };
